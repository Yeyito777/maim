...
void slop::Mouse::setVirtualButton( int button, int state ) {
    for (unsigned int i=0;i<virtualButtons.size();i++ ) {
        if ( virtualButtons[i].x == button ) {
            virtualButtons[i].y = state;
            return;
        }
    }
    virtualButtons.push_back(glm::ivec2(button,state));
}

int slop::Mouse::getButton( int button ) {
    for (unsigned int i=0;i<virtualButtons.size();i++ ) {
        if ( virtualButtons[i].x == button && virtualButtons[i].y ) {
            return virtualButtons[i].y;
        }
    }
    for (unsigned int i=0;i<buttons.size();i++ ) {
        if ( buttons[i].x == button ) {
